// src/data/inventory.ts
// ⚠️  This file is auto-generated by Studio Mode. Do not edit manually.
//     Make changes in the Studio, then click "Copy Production Config" and paste to Claude.

import type { ShadowConfig, LookbookItem, FilterDimension, AccessType } from "@/components/studio/studioTypes";

export interface OutfitItem {
  id: string;
  name: string;
  collection: string;
  colorway: string;
  /** Initiation Night price in cents, e.g. 12900 for $129 */
  initiationPriceCents: number;
  /** Member price in cents, e.g. 22900 for $229 */
  memberPriceCents: number;
  type: AccessType;
  dotPosition: string;
  lookbook?: LookbookItem[];
  filterDimensions?: FilterDimension[];
  sizes: string[];
  sizeChart?: Record<string, { chest: string; length: string }>;
  materials?: string;
  story?: string;
  sizeGuide?: string;
  /** Product thumbnail shown in the Vault listing. Path relative to /public. */
  productImage?: string;
  /** Cart thumbnail — overrides productImage in the cart drawer when set. */
  cartImage?: string;
}

export interface ModelSlot {
  id: string;
  displayName?: string;
  position: string;
  scale: string;
  mobileScale: string;
  zIndex: number;
  imageSrc: string;
  outfit: OutfitItem[];
  shadow?: ShadowConfig;
}

export const MODEL_INVENTORY: ModelSlot[] = [
  {
    id: "angel",
    displayName: "Angel",
    position: "left-[27.7%] md:left-[27.7%] bottom-[11.3%] md:bottom-[11.3%]",
    scale: "md:scale-[0.90]",
    mobileScale: "scale-[0.90]",
    zIndex: 30,
    imageSrc: "/assets/models/Angel/Angel-Heart-Long.png",
    outfit: [
      {
        id: "angel-heartbreaker",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[33.3%] left-[60.5%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "Built for the man who doesn't need to announce himself. The Constable's ivory Showstopper is the uniform of quiet authority.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Angel/Angel-Heart-Long.png",
      },
      {
        id: "angel-showstopper",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[33.3%] left-[60.5%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. The shirt that says everything without a word spoken.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Angel/Angel-pro-lit.png",
      },
      {
        id: "angel-heartbreaker-ss",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[33.3%] left-[60.5%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. Short sleeve for the man who runs warm.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Angel/Angel-pro-lit.png",
      },
      {
        id: "angel-showstopper-ls",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[33.3%] left-[60.5%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. Long sleeve for the man who commands the room through stillness.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Angel/Angel-Heart-Long.png",
      },
    ],
    shadow: { offsetX: 0, offsetY: -8, scaleX: 0.90, scaleY: 0.080, opacity: 0.45, blur: 14 },
  },
  {
    id: "jerome",
    displayName: "Jerome",
    position: "left-[4.7%] md:left-[4.7%] bottom-[15.3%] md:bottom-[15.3%]",
    scale: "md:scale-[0.84]",
    mobileScale: "scale-[0.84]",
    zIndex: 29,
    imageSrc: "/assets/models/Jerome/Jerome-Show-Short.png",
    outfit: [
      {
        id: "jerome-showstopper",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[34.5%] left-[40.1%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "Built for the man who doesn't need to announce himself. The Constable's ivory Showstopper is the uniform of quiet authority.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jerome/Jerome-Show-Short.png",
      },
      {
        id: "jerome-heartbreaker",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[34.5%] left-[40.1%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. Worn by the man who never explains himself.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jerome/Jerome-pro-lit.png",
      },
      {
        id: "jerome-showstopper-ls",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[34.5%] left-[40.1%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. Long sleeve for the man who commands attention in silence.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jerome/Jerome-pro-lit.png",
      },
      {
        id: "jerome-heartbreaker-ss",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[34.5%] left-[40.1%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. Short sleeve for the man who runs warm.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jerome/Jerome-Show-Short.png",
      },
    ],
    shadow: { offsetX: 0, offsetY: -8, scaleX: 0.90, scaleY: 0.080, opacity: 0.45, blur: 14 },
  },
  {
    id: "jack",
    displayName: "Jack",
    position: "left-[48.3%] md:left-[48.3%] bottom-[17.2%] md:bottom-[17.2%]",
    scale: "md:scale-[0.77]",
    mobileScale: "scale-[0.77]",
    zIndex: 20,
    imageSrc: "/assets/models/Jack/Jack-Show-Long.png",
    outfit: [
      {
        id: "jack-showstopper",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[34.8%] left-[44.8%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "Built for the man who doesn't need to announce himself. The Constable's ivory Showstopper is the uniform of quiet authority.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jack/Show.Long.Mid.Brick.jpeg",
      },
      {
        id: "jack-heartbreaker",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[34.8%] left-[44.8%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. The short sleeve for when restraint itself becomes the statement.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jack/Jack-pro-lit.png",
      },
      {
        id: "jack-showstopper-ss",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[34.8%] left-[44.8%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. Short sleeve for the man whose presence needs no extra fabric.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jack/Jack-pro-lit.png",
      },
      {
        id: "jack-heartbreaker-ls",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[34.8%] left-[44.8%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. Long sleeve for the man who owns every room he enters.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Jack/Jack-Show-Long.png",
      },
    ],
    shadow: { offsetX: -28, offsetY: -2, scaleX: 0.90, scaleY: 0.080, opacity: 0.35, blur: 14 },
  },
  {
    id: "ethan",
    displayName: "Ethan",
    position: "right-[4.5%] md:right-[4.5%] bottom-[13.9%] md:bottom-[13.9%]",
    scale: "md:scale-[0.87]",
    mobileScale: "scale-[0.87]",
    zIndex: 30,
    imageSrc: "/assets/models/Ethan/Ethan-Heart-Short.png",
    outfit: [
      {
        id: "ethan-heartbreaker",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[33.6%] left-[56.6%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "Built for the man who doesn't need to announce himself. The Constable's ivory Showstopper is the uniform of quiet authority.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Ethan/Heart.Short.Mid.Studio.jpeg",
      },
      {
        id: "ethan-showstopper",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[33.6%] left-[56.6%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. Long sleeve for the man who commands attention in silence.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Ethan/Ethan-pro-lit.png",
      },
      {
        id: "ethan-heartbreaker-ls",
        name: "Heartbreaker",
        collection: "The Constable",
        colorway: "Guilt Grey (Long Sleeve)",
        initiationPriceCents: 15900,
        memberPriceCents: 25900,
        type: "public",
        dotPosition: "top-[33.6%] left-[56.6%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Heartbreaker in Guilt Grey. Long sleeve for the man who owns every room he enters.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Ethan/Ethan-pro-lit.png",
      },
      {
        id: "ethan-showstopper-ss",
        name: "Showstopper",
        collection: "The Constable",
        colorway: "Ivory (Short Sleeve)",
        initiationPriceCents: 12900,
        memberPriceCents: 22900,
        type: "public",
        dotPosition: "top-[33.6%] left-[56.6%]",
        filterDimensions: [],
        lookbook: [],
        sizes: ["S", "M", "L", "XL", "XXL"],
        sizeChart: { "S": { chest: "38\"", length: "28\"" }, "M": { chest: "40\"", length: "29\"" }, "L": { chest: "42\"", length: "30\"" }, "XL": { chest: "44\"", length: "31\"" }, "XXL": { chest: "46\"", length: "32\"" } },
        story: "The Showstopper in Ivory. Short sleeve for the man whose presence needs no extra fabric.",
        materials: "98% Supima Cotton, 2% Elastane. Structured collar. Reinforced placket seam.",
        sizeGuide: "Cut close through the chest and tapered through the torso. If between sizes, size up.",
        productImage: "/assets/models/Ethan/Ethan-Heart-Short.png",
      },
    ],
    shadow: { offsetX: 0, offsetY: 0, scaleX: 0.90, scaleY: 0.080, opacity: 0.45, blur: 14 },
  },
];