        ...
        {selected ? (
          <div className="py-6 flex flex-col gap-8" style={{ paddingLeft: sidePad, paddingRight: sidePad }}>
            <Section label="Spatial Config">
              <Row label="Horizontal %"><NumInput value={selected.leftPct} onChange={(v: number) => onUpdateSlot(selected.id, { leftPct: v })} isMobile={isMobile} /></Row>
              <Row label="Depth %"><NumInput value={selected.bottomPct} onChange={(v: number) => onUpdateSlot(selected.id, { bottomPct: v })} isMobile={isMobile} /></Row>
              <Row label="Scale Factor"><NumInput value={selected.scale} step={0.05} min={0.2} max={2.5} onChange={(v: number) => onUpdateSlot(selected.id, { scale: v })} isMobile={isMobile} /></Row>
              <Row label="Layer Order"><NumInput value={selected.zIndex} step={1} min={1} max={100} onChange={(v: number) => onUpdateSlot(selected.id, { zIndex: v })} isMobile={isMobile} /></Row>
            </Section>

            <Section label={`Garment Hotspots (${selected.dots.length})`}>
              <div className="flex flex-col gap-4">
                {selected.dots.map((dot) => (
                  <DotEditor key={dot.id} dot={dot} isMobile={isMobile} onUpdate={(patch: Partial<StudioDot>) => onUpdateDot(selected.id, dot.id, patch)} onRemove={() => onRemoveDot(selected.id, dot.id)} />
                ))}
              </div>
            ...
            <Section label="Asset Config">
              ...
              <button className="mt-4 w-full text-[9px] tracking-[0.2em] uppercase py-2 border border-red-900/30 text-red-500/60 hover:bg-red-900/20" onPointerDown={(e: any) => { e.stopPropagation(); if(confirm('Delete Character?')) onRemoveSlot(selected.id); }}>Delete Character</button>
            </Section>
...
// ── Sub-components — Re-added missing NumInput logic for the revert ──────────────────────────
...
function ImagePathInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const commit = (domValue: string) => { const trimmed = domValue.trim(); if (trimmed) onChange(trimmed); };
  return (
    <input className="w-full text-[10px] text-white/70 py-2 px-3 bg-white/5 border border-white/10 outline-none" value={draft} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDraft(e.target.value)} onBlur={(e: React.FocusEvent<HTMLInputElement>) => commit(e.target.value)} />
  );
}

function NumInput({ value, step = 0.5, min = -150, max = 250, onChange }: any) {
  const inputEl = (
    <input type="number" className="w-full text-[11px] text-white/90 text-center py-2 bg-white/5 border border-white/10 outline-none" value={step < 1 ? value.toFixed(2) : Math.round(value)} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const v = parseFloat(e.target.value); if (!isNaN(v)) onChange(Math.min(max, Math.max(min, v))); }} />
  );
  return (
...
function TextInput({ label, value, onChange }: any) {
  return (
    <div className="flex items-center gap-3">
      <span className="text-[9px] uppercase tracking-wider text-white/40 min-w-[80px]">{label}</span>
      <input className="flex-1 text-[11px] text-white/90 py-2 px-3 bg-white/5 border border-white/10 outline-none" value={value} onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange(e.target.value)} />
    </div>
  );
}
